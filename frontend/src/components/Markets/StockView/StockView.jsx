...
const StockView = () => {
  const socket = useMemo(() => socketIOClient(import.meta.env.REACT_APP_STOCKS_API, { transports: ['websocket', 'polling', 'flashsocket'] }), []);
  ...
  const filteredStocks = Array.isArray(stocks) && stocks.length > 0 ? stocks.filter((stock) => {
    return stock && stock.name && stock.ticker && (
      ...
      <div className="bg-white dark:bg-gray-800">
        <div className="container mx-auto px-4 sm:px-8 w-full">
          <div className="py-8">
            ...
            {errors && errors !== "" ? (
              <div className="mt-4 text-center w-full relative inline-block px-3 py-1 font-semibold text-red-900 leading-tight">
              ...
              <span className="mt-4 text-center w-full relative inline-block px-3 py-1 font-semibold text-red-900 leading-tight">
                ...
                <span className="relative text-red-500 dark:text-red-400">
                  {Array.isArray(stocks) && stocks.length === 0 ? "No stocks available" : "No results found..."}
                </span>
              </span>
            )}
            {isListMode ? (
